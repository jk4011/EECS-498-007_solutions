...
def multiples_of_ten(start, stop):
  ...
  """
    ...
    #############################################################################
    # Replace "pass" statement with your code
    array = [i for i in range(start, stop + 1) if i % 10 == 0]
    x = torch.tensor(array, dtype=torch.float64)
    #############################################################################
    #                            END OF YOUR CODE                               #
    #############################################################################
    return x
...
def reverse_rows(x):
  ...
  """
    ...
    row_reversed = range(x.shape[0])[::-1]
    y = x[row_reversed]
    #############################################################################
    #                            END OF YOUR CODE                               #
    #############################################################################
...
def normalize_columns(x):
  ...
  """
    ...
    y = torch.clone(x)
    mean = y.mean(dim=0)
    sd = torch.sqrt(torch.sum((y - mean) ** 2, dim=0) / (y.shape[0] - 1))
